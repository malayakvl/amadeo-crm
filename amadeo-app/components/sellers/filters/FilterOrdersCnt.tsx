...
const FilterOrdersCnt: React.FC<any> = () => {
    ...
    const [countRange, setCountRange] = useState(
        filters.total_orders[0] > 0 || filters.total_orders[1] > 0 ? filters.total_orders : [0, 0]
    );
    ...
    useEffect(() => {
        if (filters.total_orders.length === 0) {
            setCountRange([0, 0]);
        } else {
            setCountRange(filters.total_orders);
        }
    }, [filters.total_orders]);
                    ...
                    <div className="block ml-2">
                        <span className="filter-label" style={{ marginLeft: '-4px' }}>
                            ...
                            <em className="float-right">
                                {countRange[0]} - {filterData.total_orders[1]}
                            </em>
                        </span>
                        <Range
                            allowCross={false}
                    ...
                    <div className="flex mt-1">
                        ...
                        <div className="w-1/2">
                            ...
                            <input
                                className="w-full form-control"
                                type="text"
                                placeholder={`${filterData.total_orders[1]}`}
                                onChange={(e) => {
                                    onSliderPriceChange([
                                        countRange[0],
                                        e.target.value.replace(/[^0-9]/g, '')
...
export default FilterOrdersCnt;
