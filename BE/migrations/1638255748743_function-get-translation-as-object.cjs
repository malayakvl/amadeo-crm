...
exports.up = pgm => {
    pgm.sql(`DROP FUNCTION IF EXISTS common__tools._get_translation_as_object;
CREATE OR REPLACE FUNCTION common__tools._get_translation_as_object(
    _schema text,
    _table text,
...
\t\t\tforeign_key,
\t\t\tlocale,
\t\t\t''"'' || locale || ''" : '' || (json_object(array_agg(column_name), array_agg(value)))::text AS translations
...
\t\tSELECT
\t\t\tforeign_key,
\t\t\t(''{'' || string_agg(translations::text, '', '') || ''}'')::json AS translations
\t\tFROM t1
\t\tGROUP BY
\t\t\tforeign_key
\t\t),
