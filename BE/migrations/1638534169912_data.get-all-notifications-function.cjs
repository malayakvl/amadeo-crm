...
CREATE OR REPLACE FUNCTION data.get_all_notifications(
    _limit integer DEFAULT 10,
    _offset integer DEFAULT 0,
    _where text DEFAULT ''::text,
    _order_by text DEFAULT ''::text
    )
    RETURNS TABLE(
        id integer,
        ...
        sender_first_name character varying,
        sender_last_name character varying
    )
    LANGUAGE plpgsql
AS $function$
DECLARE
